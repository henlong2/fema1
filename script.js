document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeGame() {
        ...
        gameMessage.textContent = '30장의 카드 중 1장의 특별한 카드를 3번의 기회 안에 찾아보세요!';
    ...
    function endGame(isWin) {
        ...
        if (isWin) {
            ...
            // 게임 승리 시 이미지가 모달로 표시되므로, 여기서는 이미지 숨김 로직을 제거
            ...
            document.querySelectorAll('.card.special-card:not(.flipped)').forEach(card => {
                card.classList.add('flipped');
                const cardInner = card.querySelector('.card-inner');
                if (cardInner) {
                    cardInner.textContent = '☀️';
